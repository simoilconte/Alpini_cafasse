...
function MembershipAlertItem({
  ...
  return (
    ...
    >
      ...
      <span
        className={`px-2 py-1 text-xs font-medium rounded-full flex-shrink-0 ${statusStyles[status]}`}
      >
        {statusLabels[status]}
      </span>
    </Link>
  ...
  return (
    <div className="space-y-3">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="flex items-center gap-3 p-3 bg-white rounded-lg border border-slate-200 animate-pulse"
        >
...
export function DashboardPage() {
  ...
  const totalCount = useQuery(api.members.getMemberCount, profile ? {} : "skip");

  // Dashboard stats - only for admin/direttivo
  const isAdmin = profile?.role === "admin" || profile?.role === "direttivo";

  const dashboardStats = useQuery(api.memberships.getDashboardStats, isAdmin ? {} : "skip");
  const unpaidMemberships = useQuery(api.memberships.getUnpaidMemberships, isAdmin ? {} : "skip");
  const expiringMemberships = useQuery(
    api.memberships.getExpiringMemberships,
    isAdmin ? {} : "skip"
  );

  // Movement stats for banner (nuova funzionalità)
  const movementStats = useQuery(api.movements.getDashboardStats, isAdmin ? {} : "skip");
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5]">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* Movimenti Banner - only for admin/direttivo */}
        {isAdmin &&
          movementStats &&
          (movementStats.currentMonth.totalMovements > 0 || movementStats.urgency.overdue > 0) && (
            <div className="mb-8">
              <Link
                to="/movimenti"
                className="block bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 hover:shadow-md transition-all"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-blue-900">Movimenti Mese Corrente</h3>
                      <div className="flex items-center gap-3 text-sm">
                        {movementStats.urgency.overdue > 0 && (
                          <span className="text-red-600 font-medium">
                            {movementStats.urgency.overdue} scadut
                            {movementStats.urgency.overdue === 1 ? "o" : "i"}
                          </span>
                        )}
                        {movementStats.urgency.due7 > 0 && (
                          <span className="text-amber-700">
                            {movementStats.urgency.due7} entro 7 giorni
                          </span>
                        )}
                        <span className="text-blue-600">
                          Entrate: €{movementStats.currentMonth.incomes.toFixed(2)}
                        </span>
                        <span className="text-red-600">
                          Uscite: €{movementStats.currentMonth.expenses.toFixed(2)}
                        </span>
                        <span
                          className={`font-medium ${movementStats.currentMonth.balance >= 0 ? "text-green-600" : "text-red-600"}`}
                        >
                          Saldo: €{movementStats.currentMonth.balance.toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                ...
                {unpaidMemberships && unpaidMemberships.length > 0 && (
                  <span className="text-sm text-slate-500">{unpaidMemberships.length} totali</span>
                )}
              </div>
