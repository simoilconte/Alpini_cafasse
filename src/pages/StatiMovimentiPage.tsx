/**
 * Stati Movimenti - Movement Statuses Management
 *
 * Configuration page for movement statuses (admin/direttivo only).
 */
...
type MovementStatus = Doc<"movementStatuses">;

export function StatiMovimentiPage() {
  const [showForm, setShowForm] = useState(false);
  const [editingStatus, setEditingStatus] = useState<MovementStatus | null>(null);

  const statuses = useQuery(api.movementStatuses.list);
  const upsertStatus = useMutation(api.movementStatuses.create);
  const updateStatus = useMutation(api.movementStatuses.update);
  const deleteStatus = useMutation(api.movementStatuses.remove);
  ...
  const openEditForm = (status: MovementStatus) => {
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <Link to="/movimenti" className="p-1 hover:bg-slate-100 rounded-lg text-slate-600">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Stati Movimenti</h1>
            <p className="text-slate-500 text-sm mt-1">
              Gestisci gli stati dei movimenti (entrate e uscite)
            </p>
          </div>
        </div>
        <button
...
export default StatiMovimentiPage;
