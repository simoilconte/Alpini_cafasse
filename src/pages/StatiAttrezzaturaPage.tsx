/**
 * Stati Attrezzatura - Equipment Status Management
 * 
 * Configuration page for equipment statuses (admin/direttivo only).
 */
...
type Status = Doc<"equipmentStatuses">;

export function StatiAttrezzaturaPage() {
  const [showForm, setShowForm] = useState(false);
  const [editingStatus, setEditingStatus] = useState<Status | null>(null);

  const statuses = useQuery(api.equipmentStatuses.list, {});
  const upsertStatus = useMutation(api.equipmentStatuses.upsert);
  const deleteStatus = useMutation(api.equipmentStatuses.remove);
  ...
  const openEditForm = (status: Status) => {
  ...
  const handleDelete = async (id: Id<"equipmentStatuses">) => {
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Stati Attrezzatura</h1>
            <p className="text-slate-500 text-sm mt-1">
              Gestisci gli stati delle attrezzature
            </p>
          </div>
        </div>
        <button
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="es. Disponibile, In uso, Manutenzione..."
...
export default StatiAttrezzaturaPage;
